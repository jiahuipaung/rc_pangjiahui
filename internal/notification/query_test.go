...
type fakeQueryRepository struct { task Task; getErr error; replayed bool; replayErr error }
func (repo *fakeQueryRepository) GetForCaller(context.Context, string, string) (Task, error) { return repo.task, repo.getErr }
func (repo *fakeQueryRepository) ReplayDeadAtomic(context.Context, string, time.Time, string) (bool, error) { return repo.replayed, repo.replayErr }
...
func TestQueryReturnsRedactedView(t *testing.T) {
	...
	if view.ID != "n-1" || view.DestinationID != "crm" || view.AttemptCount != 2 { t.Fatalf("view = %+v", view) }
}
